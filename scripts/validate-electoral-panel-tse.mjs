...
function summarizeVotes(rows) {
  ...
  for (const row of rows) {
    const cargo = row.DS_CARGO || row.CD_CARGO || 'SEM_CARGO';
    const key = String(cargo).trim();
...
const result = {
  municipality: { tseCode: MUNICIPALITY_CODE, name: MUNICIPALITY_NAME, uf: 'PR' },
...
for (const year of YEARS) {
  ...
  await download(voteUrl, voteZip);
  await download(locationUrl, locationZip);
  ...
  result.years[year] = {
    sources: { voteUrl, locationUrl },
    voteColumns: votes.headers,
    locationColumns: locations.headers,
    voteSummary: summarizeVotes(votes.rows),
    locationSummary: summarizeLocations(locations.rows),
    votes: votes.rows,
