...
export default function AdministrationSafeEnhancer() {
  ...
  const [municipalityToolbarHost, setMunicipalityToolbarHost] = useState<HTMLElement | null>(null);
  const [municipalityActionsHost, setMunicipalityActionsHost] = useState<HTMLElement | null>(null);
  ...
  const [municipalityMatchCount, setMunicipalityMatchCount] = useState(0);
  const [municipalityShownCount, setMunicipalityShownCount] = useState(0);
  ...
  const loadAccessData = useCallback(async () => {
    try {
      ...
      // Mantém a Administração original funcional caso a leitura auxiliar falhe.
    }
  }, []);
  ...
  useEffect(() => {
    ...
    const sync = () => {
      ...
      if (panel) {
        ...
        const grid = panel.querySelector<HTMLElement>(".vf-municipality-management-grid");
        if (grid) {
          ...
          setMunicipalityToolbarHost((current) => current === toolbar ? current : toolbar);
          setMunicipalityActionsHost((current) => current === actions ? current : actions);
        }
      } else {
        setMunicipalityPanel((current) => current ? null : current);
        setMunicipalityToolbarHost((current) => current ? null : current);
        setMunicipalityActionsHost((current) => current ? null : current);
      }
    };
  ...
  useEffect(() => {
    ...
    setMunicipalityMatchCount(matching.length);
    setMunicipalityShownCount(limit);
  ...
  function selectedNames(user: User) {
    const selected = new Set(drafts[user.id] || user.municipalityIds || []);
    return activeMunicipalities.filter((item) => selected.has(item.id)).map((item) => item.name);
  }
  ...
  return (
    <>
        ...
        <section className="vf-safe-gestor-admin" aria-label="Permissões dos Gestores">
          <header>
            <div>
              ...
              <p>Defina quais municípios cada Gestor pode visualizar e alternar, sem carregar centenas de opções na tela principal.</p>
            </div>
      ...
      {municipalityToolbarHost ? createPortal(
        <div className="vf-safe-municipality-toolbar">
          ...
          <small>{municipalityQuery.trim()
            ? `${municipalityMatchCount} resultado${municipalityMatchCount === 1 ? "" : "s"}`
            : `Exibindo ${municipalityShownCount} de ${municipalityMatchCount} municípios`}</small>
        </div>,
        municipalityToolbarHost,
      ) : null}

      {municipalityActionsHost && !municipalityQuery.trim() && municipalityMatchCount > PAGE_SIZE ? createPortal(
        <div className="vf-safe-municipality-actions">
          {municipalityShownCount < municipalityMatchCount ? (
            <button type="button" onClick={() => setVisibleCount((current) => Math.min(current + PAGE_SIZE, municipalityMatchCount))}>
              Ver mais 5
            </button>
          ) : null}
      ...
      {selectedGestor && createPortal(
        <div className="vf-safe-modal-backdrop" onMouseDown={(event) => { if (event.target === event.currentTarget && !saving) setSelectedGestorId(null); }}>
          <section className="vf-safe-permission-modal">
            ...
            <input className="vf-safe-permission-search" value={permissionQuery} onChange={(event) => setPermissionQuery(event.target.value)} placeholder="Buscar município" />
      ...
      <style jsx global>{`
        .vf-gestor-municipality-admin[data-vf-safe-replaced="true"] {
          display: none !important;
        }

        [data-vf-safe-gestor-host] {
        ...
        .vf-safe-gestor-admin > header small,
        .vf-safe-permission-modal > header small {
          color: #efbd43;
          font-size: 9px;
          font-weight: 900;
          letter-spacing: .13em;
        ...
        .vf-safe-gestor-admin > header h3 {
          margin: 5px 0 4px;
          color: #fff;
          font-size: 22px;
        }

        .vf-safe-gestor-admin > header p {
          margin: 0;
          max-width: 680px;
          color: #9fb3c9;
          font-size: 11px;
        ...
        .vf-safe-gestor-admin > header > span {
          min-width: 35px;
          height: 35px;
          display: grid;
          place-items: center;
        ...
        .vf-safe-admin-message {
          margin-bottom: 10px;
          padding: 9px 11px;
          border: 1px solid rgba(239,189,67,.2);
          border-radius: 10px;
          background: rgba(239,189,67,.08);
          color: #efd487;
          font-size: 10px;
        }
        ...
        .vf-safe-avatar {
          ...
          place-items: center;
          border-radius: 50%;
          background: linear-gradient(135deg,#1f6ca4,#143f6d);
          color: #fff;
          font-size: 10px;
          font-weight: 900;
        }
        ...
        .vf-safe-gestor-list > article > button {
          min-height: 36px;
          padding: 0 11px;
        ...
        .vf-admin-municipalities-panel[data-vf-safe-admin="true"] [data-vf-safe-municipality-toolbar],
        .vf-admin-municipalities-panel[data-vf-safe-admin="true"] [data-vf-safe-municipality-actions] {
          display: block !important;
          visibility: visible !important;
        }

        .vf-safe-municipality-toolbar {
          display: flex;
          align-items: center;
          gap: 9px;
          margin: 12px 0 10px;
        ...
        .vf-safe-municipality-toolbar input {
          flex: 1 1 auto;
          min-width: 0;
          min-height: 42px;
          padding: 0 12px;
        ...
        .vf-safe-municipality-actions button {
          min-height: 38px;
          padding: 0 15px;
        ...
        .vf-safe-permission-modal > header {
          display: flex;
          align-items: flex-start;
          justify-content: space-between;
          gap: 12px;
        ...
        .vf-safe-permission-modal > header > button {
        ...
        .vf-safe-permission-modal > footer {
          display: flex;
        ...
        .vf-safe-permission-modal > footer button {
          min-height: 39px;
          padding: 0 13px;
          border: 1px solid rgba(56,189,248,.2);
          border-radius: 10px;
        ...
        @media (max-width: 760px) {
          ...
          .vf-safe-gestor-admin > header p { font-size: 10px; }
