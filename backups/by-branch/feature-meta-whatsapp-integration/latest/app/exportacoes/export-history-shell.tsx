...
import "./export-history-shell.css";
...
export default function ExportHistoryShell({
  children,
  title,
  initialUser,
  initialInitials = "",
  ...
  return (
    <div className={`app-shell vf-export-official-shell ${collapsed ? "collapsed" : ""}`}>
      <div
        ...
        <nav>
          {coreMenu.map((item) => (
            <React.Fragment key={item.label}>
              <button type="button" title={item.label} onClick={() => navigate(item.href)}>
              ...
              {item.label === "WhatsApp" && (
                <>
                  ...
                  <button
                    type="button"
                    className="vf-comunicacao-sidebar-btn"
          ...
          <button
            type="button"
            className="vf-export-sidebar-btn active"
            title="Histórico de Exportações"
            onClick={() => navigate("/exportacoes")}
            aria-current="page"
          >
            <span className="nav-icon" aria-hidden="true">↧</span>
            <span className="nav-name">Histórico de Exportações</span>
          </button>
        </nav>
      ...
      <main className="main">
        <header className="topbar" data-vf-export-header="true">
          <div className="page-id">
            ...
            <div className="vf-export-header-title-wrap" title={title}>
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img
                src="/voto-forte-bandeira-icon.jpg"
                alt="Paraná"
                className="vf-export-header-logo"
              />
              <span className="vf-export-header-label">{title}</span>
            </div>
          </div>
        ...
        <section className="workspace">
          <div className="vf-export-route-content">{children}</div>
        </section>
      </main>
    </div>
  );
