...
export async function sendMetaTemplate(
  ...
  if (options.bodyParameters && options.bodyParameters.length > 0) {
    components.push({
      type: "body",
      parameters: options.bodyParameters.map((param) => ({ type: "text", text: param })),
    });
  }
  ...
  try {
    ...
    if (!res.ok) {
      const errorMsg = data?.error?.message || data?.error?.error_user_msg || `Erro Meta HTTP ${res.status}`;
...
export async function sendMetaMedia(
  ...
  if (options.caption && (options.mediaType === "image" || options.mediaType === "video" || options.mediaType === "document")) mediaObj.caption = options.caption;
...
export async function createMetaTemplate(
  ...
  if (templateData.exampleBodyParams && templateData.exampleBodyParams.length > 0) {
    bodyComponent.example = { body_text: [templateData.exampleBodyParams] };
  }
