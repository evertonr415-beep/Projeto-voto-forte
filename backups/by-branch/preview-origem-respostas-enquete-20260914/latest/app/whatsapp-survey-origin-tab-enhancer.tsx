...
const STYLE_ID = "vf-survey-origin-styles";
const PAGE_SIZE = 15;
...
function createStyles() {
  ...
  style.textContent = `
    ...
    .vf-origin-head h3{margin:0;color:#f8fafc;font-size:18px}
    .vf-origin-head p{margin:5px 0 0;color:#94a3b8;font-size:12px;line-height:1.45;max-width:690px}
    ...
    .vf-origin-kpi strong{display:block;font-size:24px;color:#f8fafc}
    .vf-origin-kpi span{display:block;margin-top:3px;color:#94a3b8;font-size:11px;font-weight:720}
    ...
    .vf-origin-filters{display:flex;gap:7px;flex-wrap:wrap}
    .vf-origin-filter{border:1px solid rgba(148,163,184,.18);background:rgba(30,41,59,.58);color:#aebbd0;border-radius:999px;padding:7px 11px;font-size:11px;font-weight:750;cursor:pointer}
    ...
    .vf-origin-list{display:grid;gap:9px}
    .vf-origin-card{padding:12px;border:1px solid rgba(148,163,184,.14);border-radius:12px;background:rgba(15,23,42,.64)}
    .vf-origin-card-head{display:flex;align-items:center;justify-content:space-between;gap:10px;margin-bottom:7px}
    .vf-origin-person{min-width:0}.vf-origin-person strong{display:block;color:#f8fafc;font-size:13px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
    .vf-origin-person span{display:block;color:#7dd3fc;font-size:10.5px;margin-top:2px}
    .vf-origin-badge{flex:none;border-radius:999px;padding:5px 8px;font-size:9px;font-weight:850}
    .vf-origin-badge.link{background:rgba(56,189,248,.12);border:1px solid rgba(56,189,248,.26);color:#7dd3fc}
    .vf-origin-badge.broadcast{background:rgba(52,211,153,.12);border:1px solid rgba(52,211,153,.27);color:#6ee7b7}
    .vf-origin-reply.wt-reply-detail{margin-top:6px;border-left:3px solid #c084fc;background:rgba(168,85,247,.12);padding:8px 10px;border-radius:7px}
    .vf-origin-raw{font-size:12px;color:#f8fafc;white-space:pre-wrap;line-height:1.45}
    .vf-origin-time{margin-top:7px;color:#64748b;font-size:10px;text-align:right}
    .vf-origin-empty{padding:30px 14px;text-align:center;border:1px dashed rgba(148,163,184,.18);border-radius:12px;color:#94a3b8;font-size:12px;line-height:1.5}
    .vf-origin-pagination{display:flex;align-items:center;justify-content:center;gap:8px;margin-top:13px}
    .vf-origin-page-btn{border:1px solid rgba(56,189,248,.25);background:rgba(15,23,42,.72);color:#cbd5e1;border-radius:8px;padding:7px 10px;font-size:11px;font-weight:750;cursor:pointer}
    .vf-origin-page-btn:disabled{opacity:.35;cursor:default}.vf-origin-page-label{color:#94a3b8;font-size:11px;text-align:center}
    ...
    @media(max-width:760px){
      ...
      #${PANEL_ID}{padding:13px;border-radius:14px}
      .vf-origin-kpis{grid-template-columns:1fr 1fr}.vf-origin-kpi:first-child{grid-column:1/-1}.vf-origin-kpi{padding:11px}.vf-origin-kpi strong{font-size:21px}
      .vf-origin-search{max-width:none;width:100%;min-width:0}.vf-origin-tools{align-items:stretch}.vf-origin-filters{width:100%;overflow-x:auto;flex-wrap:nowrap;padding-bottom:2px}.vf-origin-filter{white-space:nowrap}.vf-origin-card{padding:10px}
    }
  `;
...
function classify(responses: SurveyResponse[], broadcastPhones: Set<string>): OriginItem[] {
  return responses.map((response, index) => {
    ...
    return {
      ...response,
      id: `survey-${index}-${response.timestamp || ""}-${phone || "anon"}`,
      origin: identified ? "broadcast" : "link",
    };
  });
}

function renderPanel(panel: HTMLElement, survey: SurveyPayload, monitor: MonitorPayload, state: ViewState) {
  const broadcastPhones = new Set(
    (monitor.items || [])
      .filter((item) => realPhone(item.phone || "") && Boolean(item.sentAt))
      .map((item) => digits(item.phone || "")),
  );
  ...
  const cards = visible.length
    ? visible
        .map((item) => {
          ...
          const safeName =
            isLink || genericName === "eleitor arapongas" || genericName.includes("participante da enquete")
          ...
          const secondary = isLink
            ...
            : `${item.phone || "Contato do disparo"}${item.district && item.district !== "Não informado" ? ` · ${item.district}` : ""}`;
        ...
        .join("")
    : `<div class="vf-origin-empty">Nenhuma participação encontrada para este filtro.</div>`;

  panel.innerHTML = `
    <div class="vf-origin-head">
      <div><h3>🧭 Origem das Respostas da Enquete</h3><p>Dados existentes da enquete separados entre participações sem vínculo identificável e respostas vinculadas a um contato de disparo.</p></div>
      <button type="button" class="vf-origin-refresh">↻ Atualizar</button>
    </div>
    <div class="vf-origin-kpis">
      <div class="vf-origin-kpi"><strong>${total}</strong><span>Total de participações existentes</span></div>
      <div class="vf-origin-kpi link"><strong>${linked.length}</strong><span>Via link / sem vínculo de contato</span></div>
      <div class="vf-origin-kpi broadcast"><strong>${identified.length}</strong><span>Via disparo identificado</span></div>
    </div>
    <div class="vf-origin-tools">
      ...
      <input class="vf-origin-search" type="search" placeholder="Buscar nome, telefone, bairro ou resposta..." value="${escapeHtml(state.search)}" />
    </div>
    ...
    <div class="vf-origin-pagination">
      ...
      <span class="vf-origin-page-label">Página ${state.page} de ${totalPages} · ${filtered.length} registros</span>
      <button type="button" class="vf-origin-page-btn" data-page-action="next" ${state.page >= totalPages ? "disabled" : ""}>Próxima →</button>
    </div>
    <p class="vf-origin-note">Os registros existentes não são alterados. “Via disparo” só é usado quando já existe telefone real com envio registrado; os demais permanecem em “Via link”.</p>
  `;
}

function restoreNativeContent(parent: HTMLElement) {
  parent.querySelectorAll<HTMLElement>("[data-vf-origin-hidden='1']").forEach((node) => {
    const previous = node.dataset.vfOriginPreviousDisplay || "";
...
export default function WhatsappSurveyOriginTabEnhancer() {
  useEffect(() => {
    ...
    const detachFromOldBar = () => {
      ...
      const oldParent = currentBar.parentElement;
      if (oldParent) restoreNativeContent(oldParent);
      currentBar = null;
    };
    ...
    const sync = () => {
      ...
      if (!button || button.parentElement !== filterBar) {
        ...
        const repliesButton = Array.from(filterBar.children).find(
          (node) => node instanceof HTMLButtonElement && normalize(node.textContent || "").includes("respostas") && !normalize(node.textContent || "").includes("sem resposta"),
      ...
      if (!panel || panel.parentElement !== parent) {
        ...
        renderPanel(panel, survey, monitor, state);
      }
