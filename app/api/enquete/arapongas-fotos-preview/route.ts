...
const EVENT_TYPE = "web_poll_arapongas_photos_preview_v1";
const POLL_ID = "arapongas-photos-preview-v1";
...
function getSurveyDb() {
  ...
  if (!supabase) throw new Error("Armazenamento da enquete não configurado no ambiente de preview.");
  return supabase;
}
...
export async function GET() {
  try {
    ...
    console.error("[arapongas-photos-preview] results failed", error);
...
export async function POST(request: Request) {
  try {
    ...
    if (action === "status") return Response.json({ success: true, alreadyAnswered });
    if (action !== "submit") return Response.json({ error: "Ação inválida" }, { status: 400 });
    ...
    const { error: insertError } = await supabase.from("vf_whatsapp_events").insert({
      ...
      payload: { source: "web_poll_photos_preview", poll: POLL_ID },
    });

    if (insertError) throw insertError;

    return Response.json({ success: true, alreadyAnswered: false, ...(await getResults()) });
  } catch (error) {
    console.error("[arapongas-photos-preview] submit failed", error);
    return Response.json(
