...
function installStyles() {
  ...
  style.id = STYLE_ID;
  style.textContent = `
    .vf-unified-control{background:rgba(255,255,255,.97);border:1px solid rgba(23,52,92,.12);border-radius:14px;box-shadow:0 10px 28px rgba(15,35,65,.16);padding:10px;min-width:230px;font:600 12px/1.3 Arial,sans-serif;color:#17345c}
...
export default function MapUnifiedLayer() {
  useLayoutEffect(() => {
    ...
    const setupMap = (map: any) => {
      ...
      if (cancelled || !container?.closest(".full-map") || map._vfUnifiedLayer) return;
      ...
      control.onAdd = () => {
        ...
        node.innerHTML = `<strong>Contatos no mapa</strong><div class="vf-unified-tabs"><button type="button" data-profile="" class="active">Todos</button><button type="button" data-profile="Eleitor">Eleitores</button><button type="button" data-profile="Liderança">Lideranças</button></div><span class="vf-unified-status">Carregando…</span>`;
        ...
        node.querySelectorAll<HTMLButtonElement>("[data-profile]").forEach((button) => {
          button.addEventListener("click", () => {
            ...
            node.querySelectorAll<HTMLButtonElement>("button").forEach((item) => item.classList.toggle("active", item === button));
            void refresh();
          });
        });
        return node;
      ...
      const removeLegacyPins = () => {
        ...
        map.eachLayer?.((item: any) => {
          ...
          if (className.includes("contact-pin") || className.includes("vf-map-district-cluster") || className.includes("vf-stable-district-bubble")) removals.push(item);
        });
      ...
      const refresh = async () => {
        ...
        try {
          ...
          for (const bubble of payload.approximateDistricts || []) {
            ...
            marker.bindTooltip(`${bubble.district} · ${NUMBER.format(total)} cadastro(s)`, { direction: "top" });
            marker.bindPopup(`<div class="vf-unified-popup"><strong>${escapeHtml(bubble.district)}</strong><b>Total do bairro</b><p>${NUMBER.format(Number(bubble.voters || 0))} eleitor(es) · ${NUMBER.format(Number(bubble.leaders || 0))} liderança(s)</p><small>Posição de referência territorial. Quem tem coordenada exata também aparece como pino individual.</small></div>`, { maxWidth: 290 });
            marker.addTo(layer);
            bubbleCount += 1;
          }
          ...
          for (const feature of payload.features || []) {
            ...
            if (feature.feature_type === "cluster" || total > 1) {
              const marker = L.marker([lat, lon], {
                zIndexOffset: 800,
                icon: L.divIcon({ className: "vf-unified-cluster", html: `<span>${NUMBER.format(Math.max(2, total))}</span>`, iconSize: [42, 42], iconAnchor: [21, 21] }),
              });
              marker.bindTooltip(`${NUMBER.format(Number(feature.voters || 0))} eleitor(es) · ${NUMBER.format(Number(feature.leaders || 0))} liderança(s)`, { direction: "top" });
              marker.addTo(layer);
              continue;
            }
            ...
            const marker = L.marker([lat, lon], {
              zIndexOffset: 900,
              icon: L.divIcon({ className: "vf-unified-person", html: `<span class="vf-unified-pin ${isLeader ? "leader" : "voter"}"><i>${isLeader ? "L" : "•"}</i></span>`, iconSize: [32, 36], iconAnchor: [15, 32], popupAnchor: [0, -30] }),
            });
            const address = [feature.street, feature.street_number].filter(Boolean).join(", ");
            marker.bindPopup(`<div class="vf-unified-popup"><strong>${escapeHtml(feature.contact_name || "Contato")}</strong><b>${escapeHtml(feature.profile || "Eleitor")}</b>${feature.district ? `<p>${escapeHtml(feature.district)}</p>` : ""}${address ? `<p>${escapeHtml(address)}</p>` : ""}</div>`, { maxWidth: 260 });
            marker.addTo(layer);
          }

          const stats = payload.stats;
          if (statusNode && stats) statusNode.textContent = `${bubbleCount} bairros · ${NUMBER.format(stats.mappedContacts)} pino(s) exato(s)`;
          const message = document.querySelector<HTMLElement>(".real-map-toolbar strong");
          if (message) message.textContent = `${bubbleCount} bairro(s) com bolhas no mapa`;
        } catch {
          if (statusNode) statusNode.textContent = "Não foi possível carregar agora";
        }
      };
      ...
      const cleanup = () => {
        ...
        try { map.removeLayer(layer); map.removeControl(control); } catch {}
    ...
    const patchLeaflet = () => {
      const L = (window as any).L;
      if (!L?.map || L.map.__vfUnifiedLayerPatched) return false;
      ...
      wrappedMap.__vfUnifiedLayerPatched = true;
      L.map = wrappedMap;
      return true;
    };
    ...
    return () => {
      ...
      const L = (window as any).L;
      if (L?.map?.__vfUnifiedLayerPatched && originalMapFactory) L.map = originalMapFactory;
    };
  }, []);
