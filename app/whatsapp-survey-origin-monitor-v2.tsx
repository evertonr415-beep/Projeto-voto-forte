...
const STYLE_ID = "vf-survey-origin-styles-v2";
const PAGE_SIZE = 15;
...
function buildItems(survey: SurveyPayload, identifiedPayload: IdentifiedPayload): OriginItem[] {
  ...
  for (const response of Array.isArray(survey.responses) ? survey.responses : []) {
    const phone = digits(response.phone || "");
    const identified = realPhone(response.phone || "") && !genericName(response.contactName || "");
    if (identified && phone) broadcastPhones.add(phone);
    items.push({
      ...response,
      id: `survey-${items.length}-${response.timestamp || ""}-${phone || "anon"}`,
      origin: identified ? "broadcast" : "link",
    });
  }

  for (const response of Array.isArray(identifiedPayload.responses) ? identifiedPayload.responses : []) {
    const phone = digits(response.phone || "");
    if (!phone || broadcastPhones.has(phone)) continue;
    broadcastPhones.add(phone);
    items.push({
      ...response,
      id: `monitor-${phone}-${response.timestamp || items.length}`,
      origin: "broadcast",
    });
  }
...
function renderPanel(panel: HTMLElement, survey: SurveyPayload, identifiedPayload: IdentifiedPayload, state: ViewState) {
  ...
  const cards = visible.length
    ? visible.map((item) => {
        ...
        const secondary = isLink
          ...
          : `${item.phone || "Contato identificado"}${item.district ? ` · ${item.district}` : ""}`;
  ...
  panel.innerHTML = `
    <div class="vf-origin-head"><div><h3>🧭 Origem das Respostas da Enquete</h3><p>Reúne as respostas anônimas do link e também as respostas que o próprio Monitor já consegue vincular a um contato do disparo.</p></div><button type="button" class="vf-origin-refresh">↻ Atualizar</button></div>
    <div class="vf-origin-kpis"><div class="vf-origin-kpi"><strong>${items.length}</strong><span>Registros de resposta nesta visão</span></div><div class="vf-origin-kpi link"><strong>${linked.length}</strong><span>Via link / sem identificação</span></div><div class="vf-origin-kpi broadcast"><strong>${identified.length}</strong><span>Via disparo identificado</span></div></div>
    ...
    <p class="vf-origin-note">Esta tela não altera votos nem resultados. “Via disparo” usa contatos com telefone real, envio anterior registrado e resposta relacionada à pesquisa; “Via link” mantém as participações sem vínculo seguro com uma pessoa.</p>
  `;
}
...
export default function WhatsappSurveyOriginMonitorV2() {
  useEffect(() => {
    createStyles();
    let active = false;
    let survey: SurveyPayload = {};
    let identified: IdentifiedPayload = {};
    ...
    const draw = () => {
      ...
      if (panel) renderPanel(panel, survey, identified, state);
    };
    ...
    const load = async () => {
      ...
      try {
        ...
        if (identifiedResponse.ok && identifiedData?.success) identified = identifiedData as IdentifiedPayload;
      } finally {
        loading = false;
        draw();
      }
    ...
    const sync = () => {
      ...
      if (currentBar && currentBar !== filterBar) detach();
    ...
    const onClick = (event: MouseEvent) => {
      ...
      if (filterButton) {
        ...
        if (filter === "all" || filter === "link" || filter === "broadcast") { state.filter = filter; state.page = 1; draw(); }
        return;
      }
