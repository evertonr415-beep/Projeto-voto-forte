...
export default function LocationIssuesClient({ currentUser }: { currentUser: CurrentUser }) {
  ...
  async function deleteSelected() {
    ...
    try {
      ...
      setMessage(`${Number(payload.deleted || ids.length).toLocaleString("pt-BR")} contato(s) excluído(s).`);
      setSelectedIds(new Set());
      await load();
    } catch (error) {
    ...
    <main className="issues-shell">
      <header className="issues-header">
        ...
        <div className="issues-actions">
          ...
          {isAdmin && (
            <select value={scope} onChange={(event) => { setScope(event.target.value); setPage(1); clearFilters(); }}>
      ...
      <section className="issues-kpis">
        ...
        <article><b>{qualityPercent}%</b><span>Base sem pendências</span></article>
      </section>
      ...
      <section className="issues-panel">
        <div className="issues-toolbar">
          ...
          <div className="issues-toolbar-controls">
            <input value={queryInput} onChange={(event) => setQueryInput(event.target.value)} placeholder="Buscar nome, telefone ou local" />
              ...
              <tbody>
                {data.issues.map((issue) => (
                  <tr key={issue.record_id}>
                    <td><input type="checkbox" checked={selectedIds.has(issue.record_id)} onChange={() => toggleIssue(issue.record_id)} aria-label={`Selecionar ${issue.contact_name}`} /></td>
      ...
      {message && <div className="issues-toast" role="status" onClick={() => setMessage("")}>{message}</div>}
      {editing && (
        <div className="issues-modal-backdrop" onMouseDown={() => setEditing(null)}>
          <section className="issues-modal" onMouseDown={(event) => event.stopPropagation()}>
            <header><div><small>ANÁLISE DO CONTATO</small><h2>{editing.contact_name}</h2></div><button onClick={() => setEditing(null)}>×</button></header>
            ...
            {editing.issue_codes.some((code) => ["missing_location", "incomplete_location", "location_divergence"].includes(code)) && (
            ...
            <footer><button onClick={() => setEditing(null)}>Fechar</button><button className="primary" disabled={!selectedDistrict} onClick={() => void saveDistrict()}>Salvar e reanalisar</button></footer>
          </section>
        </div>
      )}
    </main>
