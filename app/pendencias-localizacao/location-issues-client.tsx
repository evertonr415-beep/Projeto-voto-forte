    ...
    <main className="issues-shell">
      <header className="issues-header">
        ...
        <div className="issues-actions">
          ...
          {isAdmin && (
            <select value={scope} onChange={(event) => { setScope(event.target.value); setPage(1); setCategory(""); }}>
      ...
      <section className="issues-panel">
        ...
        {loading ? <div className="issues-loading">Carregando pendências…</div> : (
          <div className="issues-table-wrap">
            <table>
              <thead><tr><th>Contato</th><th>Telefone</th><th>Valor importado</th><th>Motivo</th><th>Sugestão</th>{isAdmin && <th>Responsável</th>}<th>Ação</th></tr></thead>
              <tbody>
                {data.issues.map((issue) => (
                  <tr key={issue.record_id}>
                    <td>{issue.contact_name || "Sem nome"}</td><td>{issue.phone}</td><td>{issue.district_original || "—"}</td>
                    <td><span className={`issue-tag ${issue.category}`}>{CATEGORY_LABELS[issue.category] || issue.category}</span></td>
                    <td>{issue.suggested_district || "—"}</td>{isAdmin && <td>{issue.owner_email}</td>}
        ...
        <footer className="issues-pagination">
          ...
          <span>Página {data.page} de {data.totalPages} · {data.total.toLocaleString("pt-BR")} registros no filtro</span>
      ...
      {editing && (
        <div className="issues-modal-backdrop" onMouseDown={() => setEditing(null)}>
          <section className="issues-modal" onMouseDown={(event) => event.stopPropagation()}>
            <header><div><small>CORRIGIR LOCALIZAÇÃO</small><h2>{editing.contact_name}</h2></div><button onClick={() => setEditing(null)}>×</button></header>
            ...
            {editing.suggested_district && <button className="issues-suggestion" onClick={() => setSelectedDistrict(editing.suggested_district || "")}>Usar sugestão: {editing.suggested_district}</button>}
            <footer><button onClick={() => setEditing(null)}>Cancelar</button><button className="primary" disabled={!selectedDistrict} onClick={() => void saveDistrict()}>Salvar correção</button></footer>
          </section>
        </div>
      )}
    </main>
