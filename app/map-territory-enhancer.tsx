  ...
  return `
    <div class="vf-district-point-wrap" aria-label="${escapeHtml(item.district)} (Arapongas): ${NUMBER.format(item.total)} contatos">
      <div class="vf-district-point-box">
        ...
        ${collegeCount > 0 ? `
          <button type="button" class="vf-district-college-btn" data-college-id="${mainCollege.id}" data-district="${escapeHtml(item.district)}" title="Município: Arapongas - PR · Colégio: ${escapeHtml(mainCollege.shortName || mainCollege.name)} (${NUMBER.format(mainCollege.totalVoters)} eleitores) - Clique para ver apuração TSE">
...
function installStyles() {
  ...
  style.id = STYLE_ID;
  style.textContent = `
    /* BLOQUEIO DE MARCADORES ANTIGOS DE COLÉGIOS SOLTOS NO MAPA */
...
export default function MapTerritoryEnhancer() {
  useLayoutEffect(() => {
    ...
    const setupMap = (map: any) => {
      ...
      // Limpeza ativa de quaisquer marcadores antigos residuais
      try {
        const strayNodes = document.querySelectorAll(".vf-polling-place-marker-icon, .vf-map-college-point");
